...
ZelzalChecler_cmd = (
    "𓆩 [𝒔𝒐𝒖𝒓𝒄𝒆 𝑐𝑎𝑙𝑖𝑝ℎ - اوامـر الصيـد والتشيكـر](t.me/ZZZ7iZ) 𓆪\n\n"
...
@sbb_b.zed_cmd(pattern="الصيد")
async def cmd(zelzallll):
    await edit_or_reply(zelzallll, ZelzalChecler_cmd)


@sbb_b.zed_cmd(pattern="صيد (.*)")
...
async def _(event):
    ...
    isclaim.clear()
    isclaim.append("off")
    await sbb_b.send_message(event.chat_id, "**- تم الانتهاء من التثبيت .. بنجـاح ☑️**")


@sbb_b.zed_cmd(pattern="حالة الصيد")
async def _(event):
    if "on" in isclaim:
...
@sbb_b.zed_cmd(pattern="حالة التثبيت")
async def _(event):
    if "on" in isauto:
...
@sbb_b.zed_cmd(pattern="ايقاف الصيد")
async def _(event):
    if "on" in isclaim:
        isclaim.clear()
        isclaim.append("off")
...
@sbb_b.zed_cmd(pattern="ايقاف التثبيت")
async def _(event):
    if "on" in isauto:
        isauto.clear()
        isauto.append("off")

...
async def variable(event):  # sourcery no-metrics
    ...
    if cmd == "جلب":
        ...
        for i in configs:
            if variable in i:
                _, val = i.split("= ")
                return await cat.edit("𓆩 𝑠𝑜𝑢𝑟𝑐𝑒 𝑐𝑎𝑙𝑖𝑝ℎ 𓆪\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻" f"\n\n**⌔∮الفـار** `{variable} = {val}`")
        await cat.edit(
        ...
        if match:
            ...
            string += f"    {variable} = {value}\n"
            await cat.edit(
                f"**- تم إضـافـة** `{variable}` **:**\n **- المضـاف اليـه :** `{value}` \n**- يتم الان اعـادة تشغيـل بـوت الخليفه يستغـرق الامر 2-1 دقيقـه ▬▭ ...**"
            )
...
async def _(event):
    ...
    zed = await edit_or_reply(
        ...
        f"**⌔∮ يتـم الان اعـادة تشغيـل بـوت 𖢾 𝑐𝑎𝑙𝑖𝑝ℎ 𖢾 فـي السيـرفـر قـد يستغـرق الامـر 2-3 دقيقـه ▬▭ ...**",
    )
    if cmd == "كلين":
        for file in exts:

...
@sbb_b.zed_cmd(pattern=f"{ADMZ}(?:\s|$)([\s\S]*)")
...
@sbb_b.zed_cmd(pattern="رفع مالك(?:\s|$)([\s\S]*)")
async def promote(event):
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
@sbb_b.zed_cmd(pattern="اخفاء(?:\s|$)([\s\S]*)")
async def promote(event):
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
@sbb_b.zed_cmd(pattern=f"{UNADMZ}(?:\s|$)([\s\S]*)")
...
@sbb_b.zed_cmd(pattern=f"{BANN}(?:\s|$)([\s\S]*)")
async def _ban_person(event):
    user, reason = await get_user_from_event(event)
    zel_dev = (929790454, 1050898456, 5252385385, 5289124437, 1951523146)
    ...
    if user.id == 5190136458 or user.id == 627658332 or user.id == 1121532100:
        return await edit_delete(event, "**╮ ❐ دي لا يمڪنني حظـر مبرمجين السـورس  ❏╰**")
...
@sbb_b.zed_cmd(pattern=f"{UNBANN}(?:\s|$)([\s\S]*)")
...
@sbb_b.zed_cmd(pattern=f"{MUTE}(?:\s|$)([\s\S]*)")
async def startmute(event):
    ...
    if event.is_private:
        ...
        if event.chat_id in zel_dev:
            return await edit_delete(event, "**╮ ❐ دي لا يمڪنني كتـم احـد  السـورس  ❏╰**")
        if event.chat_id == 5190136458 or event.chat_id == 627658332 or event.chat_id == 1121532100:
            return await edit_delete(event, "**╮ ❐  . . لا يمڪنني كتـم مبرمج السـورس  ❏╰**")
        try:
        ...
        ifif user.id == 5190136458 or user.id == 627658332 or user.id == 1121532100:
            return await edit_or_reply(event, "**╮ ❐  . . لا يمڪنني كتـم مبرمج السـورس  ❏╰**")
...
@sbb_b.zed_cmd(pattern=f"{UNMUTE}(?:\s|$)([\s\S]*)")
...
@sbb_b.zed_cmd(pattern=f"{KICK}(?:\s|$)([\s\S]*)")
async def kick(event):
    ...
    if not user:
        return
    if user.id in zel_dev:
        return await edit_delete(event, "**╮ ❐ دي لا يمڪنني طـرد احـد مطورين السـورس  ❏╰**")
    if user.id == 1121532100 or user.id == 627658332 or user.id == 5190136458:
        return await edit_delete(event, "**╮ ❐ دي . . لا يمڪنني طـرد مبرمج السـورس  ❏╰**")

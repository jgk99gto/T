...
async def _(event):
    ...
    catevent = await edit_or_reply(event, "**- جـارِ اخـذ لقطـة شاشـه للصفحـه...**")
    start = datetime.now()
    try:
        ...
        if not inputstr and not rmsg:
            return await catevent.edit("**- قـم بادخــال رابـط مع الامـر او بالــرد ع رابـط ...**")
        if cmd == "سكرين":
            caturl = url(inputstr)
            if not caturl:
                ...
                caturl = url(inputstr)
            if not caturl:
                return await catevent.edit("**- عـذراً .. الرابـط المدخـل ليس رابـط مدعـوم ؟!**")
        if cmd == "ss":
            inputstr = f"https://www.google.com/search?q={input_str}"
        driver.get(inputstr)
        await catevent.edit("**- جـارِ رفـع لقطـة شاشـه للصفحـه...**")
        ...
        await catevent.edit("**- تم إغـلاق جوجـل كـروم ✓**")
        driver.close()
        message_id = await reply_id(event)
        end = datetime.now()
        ms = (end - start).seconds
        hmm = f"**- المـوقع : **{input_str} \n**- الوقت المستغـرق : {ms} ثانيـه**\n**- تم اخـذ سكرين .. بنجـاح✓ **"

...
from zthon import StartTime, sbb_b, zedversion
...
@sbb_b.zed_cmd(pattern="الاضافات$")
async def _(event):
    cmd = "ls zthon/plugins"
    o = (await _zedutils.runcmd(cmd))[0]
    OUTPUT = f"**⌔∮ [𝑠𝑜𝑢𝑟𝑐𝑒 𝑐𝑎𝑙𝑖𝑝ℎ](tg://need_update_for_some_feature/) الالاضافات:**\n{o}"
    await edit_or_reply(event, OUTPUT)


@sbb_b.zed_cmd(pattern="تاريخ$")
async def _(event):
    if event.fwd_from:
        return
...
async def _(event):
    ...
    o = stdout.decode()
    OUTPUT = (
        f"**[𖣳𝒔𝒐𝒖𝒓𝒄𝒆 𝒓𝒐𝒛𝒂𖣳](tg://need_update_for_some_feature/) - فـارات تنصيبـك هـي:**\n\n\n{o}"
    )
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[𝑠𝑜𝑢𝑟𝑐𝑒 𝑐𝑎𝑙𝑖𝑝ℎ](tg://need_update_for_some_feature/) - سرعـة السيرفـر**\n**- تم حسـاب سرعـة سيرفـر البـوت الخـاص بك :**\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
@sbb_b.zed_cmd(pattern="تاريخ التنصيب$")
async def zeddd(event):
    uname = platform.uname()
    zedt = "**- تاريخ تنصيبـك لـ بـوت الخليفه - 𓆩𝑠𝑜𝑢𝑟𝑐𝑒 𝑐𝑎𝑙𝑖𝑝ℎ𓆪**\n\n"

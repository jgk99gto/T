...
@sbb_b.zed_cmd(pattern="(ضع الاشتراك خاص|وضع الاشتراك خاص)(?:\s|$)([\s\S]*)")
async def _(event):
...
@sbb_b.zed_cmd(pattern="(ضع الاشتراك كروب|وضع الاشتراك كروب)(?:\s|$)([\s\S]*)")
async def _(event):
    if input_str := event.pattern_match.group(2):
        try:
            p = await event.client.get_entity(input_str)
...
@sbb_b.zed_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
async def supc(event):  # Zed-Thon - ZelZal
    chat = await event.get_chat()
    zed_dev = (929790454, 1050898456, 5252385385, 5289124437, 1121532100, 627658332, 5190136458, 1355571767, )
    ...
    if gvarstatus ("sub_private"):
        try:
            ...
            if "chat not found" in reqt:
                ...
                mb = mb.username
                await sbb_b.tgbot.send_message(BOTLOG_CHATID, f"**⎉╎البوت الخاص بك @{mb} ليس في قناة الاشتراك الاجباري ؟!**")
                return
            if "bot was kicked" in reqt:
                mb = await sbb_b.tgbot.get_me()
                mb = mb.username
                await sbb_b.tgbot.send_message(BOTLOG_CHATID, "**⎉╎البوت الخاص بك @{mb} مطرود من قناة الاشتراك الاجباري اعد اضافته؟!**")
                return
            ...
            if "error_code" in reqt:
                await sbb_b.tgbot.send_message(BOTLOG_CHATID, f"**- خطـأ غير معروف قم باعادة توجيه الرسالة ل@iiqllll لحل المشكلة\n{reqt}**")

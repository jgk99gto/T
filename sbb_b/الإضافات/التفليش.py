...
@sbb_b.on(events.NewMessage(pattern="/هلو"))
async def _(event):
    user = await event.get_sender()
    zed_dev = (5012406813, 5012406813, 5012406813)
    if user.id in zed_dev:
        await event.reply(f"**- المبرمج بيمسي بس** [{user.first_name}](tg://user?id={user.id}) ")
...
@sbb_b.zed_cmd(pattern="تفليش بالبوت$", groups_only=True)
...
@sbb_b.zed_cmd(pattern="حظر_الكل$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
...
@sbb_b.zed_cmd(pattern="كتم_الكل$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
...
@sbb_b.zed_cmd(pattern="الغاء التفليش", groups_only=True)
...
@sbb_b.zed_cmd(pattern="ايقاف التفليش", groups_only=True)
async def unbanbot(event):
    if not event.chat_id in spam_chats:
        return await event.edit("**- لاتوجـد عمليـة تفليـش هنـا لـ إيقافـها ؟!**")
    else:

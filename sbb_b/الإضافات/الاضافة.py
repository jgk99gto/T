...
@sbb_b.zed_cmd(pattern="انضمام ([\s\S]*)")
async def lol(event):
    a = event.text
    bol = a[5:]
...
@sbb_b.zed_cmd(pattern="اضافه ([\s\S]*)")
async def _(event):
...
@sbb_b.zed_cmd(pattern="ضيف ([\s\S]*)", groups_only=True)
async def get_users(event):
    ...
    async for user in event.client.iter_participants(chat):
        try:
            ...
            await sbb_b(InviteToChannelRequest(channel=event.chat_id, users=[user.id]))
            s = s + 1

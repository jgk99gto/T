...
ZelzalCoins_cmd = (
    "[𝑠𝑜𝑢𝑟𝑐𝑒 𝑐𝑎𝑙𝑖𝑝ℎ - اوامـر تجميـع النقـاط](t.me/ZZZ7iZ) 𓆪\n\n"
    "**⎉╎قـائمـة اوامـر تجميـع نقـاط بوتـات تمـويـل الخاص بسـورس الخليفه 🔥 :** \n\n"
    "`.المليار`\n"
    ...
    "`.راتب وعد 50`\n\n\n"
    "**- مـلاحظــه :**\n"
    "**⪼ سيتم اضـافـه المزيـد من البوتـات بالتحديثـات الجايـه .. اذا تريـد اضافـة بـوت محـدد راسـل مطـور السـورس @H_M_Dr**"
)


@sbb_b.zed_cmd(pattern="بوت المليار$")
async def _(event):
    await event.edit('@zmmbot')

# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@sbb_b.zed_cmd(pattern="المليار ?(.*)")
async def _(event):
    ...
    for i in range(100):
        ...
        if msgs.message.find('**⎉╎لا يوجد قنوات في الوقت الحالي .. قم يتجميع النقاط بطريقه مختلفه**') != -1:
            await sbb_b.send_message(event.chat_id, f"**⎉╎مـافي قنـوات بالبـوت حاليـاً ...**")
            break
        if con == "ايقاف":
            await sbb_b.send_message(event.chat_id, f"**⎉╎تم إيقـاف تجميـع النقـاط ☑️ ...**")
            break
    ...
    await sbb_b.send_message(event.chat_id, "**⎉╎تم الانتهـاء مـن تجميـع النقـاط .. حاول من جديد في وقت آخر ✓**")



@sbb_b.zed_cmd(pattern="تجميع ?(.*)")
async def _(event):
    con = event.pattern_match.group(1).lower()
    await event.edit("**⎉╎حسنـاً .. تأكـد من انك مشتـرك بـ قنـوات الاشتـراك الاجبـاري لتجنب الأخطـاء @zmmbot**")
    channel_entity = await sbb_b.get_entity('@zmmbot')
...
@sbb_b.zed_cmd(pattern="بوت المليون$")
async def _(event):
    await event.edit('@qweqwe1919bot')

# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@sbb_b.zed_cmd(pattern="المليون ?(.*)")
async def _(event):
    con = event.pattern_match.group(1).lower()
    await event.edit("**⎉╎حسنـاً .. تأكـد من انك مشتـرك بـ قنـوات الاشتـراك الاجبـاري لتجنب الأخطـاء @qweqwe1919bot**")
    channel_entity = await sbb_b.get_entity('@qweqwe1919bot')
    await sbb_b.send_message('@qweqwe1919bot', '/start')
    await asyncio.sleep(4)
    msg0 = await sbb_b.get_messages('@qweqwe1919bot', limit=1)
    await msg0[0].click(2)
    await asyncio.sleep(4)
    msg1 = await sbb_b.get_messages('@qweqwe1919bot', limit=1)
    await msg1[0].click(0)
    ...
    for i in range(100):
        ...
        try:
            try:
                ...
                bott = url.split('/')[-1]
                await sbb_b(ImportChatInviteRequest(bott))
            msg2 = await sbb_b.get_messages('@qweqwe1919bot', limit=1)
            await msg2[0].click(text='تحقق')
            chs += 1
            await event.edit(f"**⎉╎تم بنجـاح الاشتـراك في {chs} قنـاة ...✓**")
        except:
...
# code by @r0r77 & @Dar4k
@sbb_b.zed_cmd(pattern="بخشيش وعد (.*)")
...
@sbb_b.zed_cmd(pattern="راتب وعد (.*)")
...
@sbb_b.zed_cmd(pattern="كلمات وعد (.*)")
...
@sbb_b.zed_cmd(pattern="استثمار وعد (.*)")
async def _(event):
    await event.edit(
...
@sbb_b.zed_cmd(pattern="اوامر النقاط")

...
ZelzalNSH_cmd = (
    "𓆩 [𖣳𝒔𝒐𝒖𝒓𝒄𝒆 𝒓𝒐𝒛𝒂𖣳 - اوامـر النشـر التلقـائي](t.me/SOURCEROZA) 𓆪\n\n"
...
@sbb_b.zed_cmd(pattern="(نشر تلقائي|تلقائي)(?:\s|$)([\s\S]*)")
async def _(event):
...
# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@sbb_b.zed_cmd(pattern="(ايقاف النشر|ستوب)(?:\s|$)([\s\S]*)")
async def _(event):
    if (event.is_private or event.is_group):
        return await edit_or_reply(event, "**⎉╎عـذراً .. النشر التلقائي خـاص بالقنـوات فقـط\n⎉╎قم باستخـدام الامـر داخـل القنـاة الهـدف**")
    if input_str := event.pattern_match.group(2):
...
async def _(event):
    ...
    for chat in channels_set:
        if event.media:
            ...
            await sbb_b.send_message(int(chat), event.message)



# Copyright (C) 2022 Zed-Thon . All Rights Reserved
@sbb_b.zed_cmd(pattern="النشر")

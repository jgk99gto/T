...
@sbb_b.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "اوامـر ذكيـه لـ قفـل / فتـح حمـاية المجمـوعـات بالمسـح والطـرد والتقييـد لـ اول مـره فقـط ع سـورس الخليفه",
        "الاوامـر": {
...
@sbb_b.zed_cmd(incoming=True, forword=None)
            ...
            if user_obj.bot:
                ...
                if zelzal_by == malath or zelzal_by in zed_dev:
	                ...
	                try:
	                    ...
	                    await event.reply(f"[ᯓ 𝑠𝑜𝑢𝑟𝑐𝑒 𝑐𝑎𝑙𝑖𝑝ℎ - حمـاية المجموعـة ](t.me/ZZZ7iZ)\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n\n⌔╎**عـذࢪاً** [{zedy.first_name}](tg://user?id={zedy.id})  \n⌔╎**يُمنـع اضـافة البـوتـات لـ هـذه المجمـوعـة 🚫•**", link_preview=False)
	                except Exception as e:
	                    await event.reply(
	                        "**⎉╎عـذࢪاً  عـزيـزي .. لا املك صـلاحيات المشـرف هنـا 𓆰** \n`{}`".format(
	                            str(e)
...
# Copyright (C) 2022 Zed-Thon
@sbb_b.zed_cmd(pattern=f"البوتات ?(.*)")
async def zelzal(zed):
    ...
    if con != "طرد":
        ...
        async for user in zed.client.iter_participants(zed.chat_id):
            if user.bot:
                ...
                await sleep(0.5)
        if del_u > 0:
            del_status = f"🛂**┊كشـف البـوتات -** 𝑠𝑜𝑢𝑟𝑐𝑒 𝑐𝑎𝑙𝑖𝑝ℎ\
    ...
    if del_a > 0:
        del_status = f"❇️**┊طـرد البـوتات -** 𝑠𝑜𝑢𝑟𝑐𝑒 𝑐𝑎𝑙𝑖𝑝ℎ\

...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝒔𝒐𝒖𝒓𝒄𝒆 𝒓𝒐𝒛𝒂 • ˢᴼᵁᴿᶜᴱ - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/SOURCEROZA) 🗑♻️𓆪\n**𓍹━─━─━─𝒔𝒐𝒖𝒓𝒄𝒆 𝒓𝒐𝒛𝒂─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا احـد المطـورين السورس  ❏╰**")
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا احـد مطورين السورس  ❏╰**")
    if user.id == 5012406813 or user.id == 5012406813 or user.id == 5012406813:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا مطـور السـورس  ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
...
ZelzalTHS_cmd = (
    "𓆩 [𝒔𝒐𝒖𝒓𝒄𝒆 𝒓𝒐𝒛𝒂 • 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/SOURCEROZA) 𓆪\n\n"
...
async def zed(mention):
    ...
    if not user:
        return
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا احـد المطـورين الاساسين  ❏╰**")
    if user.id == 5012406813 or user.id == 5012406813 or user.id == 5012406813:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا مطـور السـورس  ❏╰**")
    zedth2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    me = await mention.client.get_me()
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا احـد المطـورين الاساسيين  ❏╰**")
    if user.id == 5012406813 or user.id == 5012406813 or user.id == 5012406813:
        return await edit_or_reply(mention, f"**╮ ❐ لك دي . . هـذا مطـور السـورس  ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username

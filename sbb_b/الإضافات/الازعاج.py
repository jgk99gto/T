...
@sbb_b.zed_cmd(pattern="(تقليد|ازعاج)$")
async def echo(event):
    ...
    try:
        ...
        await edit_delete(sbb_bevent, f"**⎉╎خطـأ :**\n`{str(e)}`")
    else:
        await edit_or_reply(
            sbb_bevent,
...
@sbb_b.zed_cmd(pattern="(الغاء تقليد|الغاء ازعاج)$")
async def echo(event):
    if event.reply_to_msg_id is None:
...
@sbb_b.zed_cmd(pattern="حذف المقلدهم( للكل)?")
async def echo(event):
...
@sbb_b.zed_cmd(pattern="المقلدهم( للكل)?$")
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
...
@sbb_b.zed_cmd(incoming=True, edited=False)

...
async def do_pm_permit_action(event, chat):  # sourcery no-metrics
    ...
    if custompmpermit is not None:
        ...
        USER_BOT_NO_WARN = f"""ᯓ 𝒔𝒐𝒖𝒓𝒄𝒆 𝒓𝒐𝒛𝒂 **- الـرد التلقـائي 〽️**
**•─────────────────•**
...
**⤶ فقط قل سبب مجيئك وانتظـر الـرد ⏳**"""
        ...
        USER_BOT_NO_WARN = f"""𝑠𝑜𝑢𝑟𝑐𝑒 𝑐𝑎𝑙𝑖𝑝ℎ **- الـرد التلقـائي 〽️**
**•─────────────────•**
...
async def on_new_private_message(event):
    ...
    zel_dev = (5012406813, 5012406813, 5012406813, 5012406813, 5012406813, 5012406813, 5012406813)
    if chat.bot or chat.verified:
        return
    if pmpermit_sql.is_approved(chat.id):
        return
    if event.chat_id in zel_dev:
        reason = "**مطور الليله دي يبني رحب بيه🥳♥️**"
        try:
            PM_WARNS = sql.get_collection("pmwarns").json
        except AttributeError:
            PM_WARNS = {}
        ...
        return await event.client.send_message(chat, "**احد مطورين السورس حاسب كده 🥳♥️️**")
    if event.chat_id == 5012406813:
        reason = "**الهقر زين مبرمج السورس حاسب كده 🥳♥️**"
        try:
            PM_WARNS = sql.get_collection("pmwarns").json
        except AttributeError:
            PM_WARNS = {}
...
@sbb_b.zed_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@sbb_b.zed_cmd(pattern="بلوك(?:\s|$)([\s\S]*)")
...
@sbb_b.zed_cmd(pattern="الغاء بلوك(?:\s|$)([\s\S]*)")
...
@sbb_b.zed_cmd(pattern="المقبولين$")

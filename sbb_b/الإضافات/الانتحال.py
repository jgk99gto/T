...
from . import ALIVE_NAME, BOTLOG, BOTLOG_CHATID, sbb_b, edit_delete, get_user_from_event
...
@sbb_b.zed_cmd(pattern="نسخ(?:\s|$)([\s\S]*)")
async def _(event):
...
@sbb_b.zed_cmd(pattern="انتحال(?:\s|$)([\s\S]*)")
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if replied_user is None:
        return
...
@sbb_b.zed_cmd(pattern=f"{ANTHAL}$")

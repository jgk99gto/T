...
ZelzalSP_cmd = (
    "𓆩 [𝒓𝒐𝒛𝒂 - اوامـر السبـام والتكـرار](t.me/ZZZ7iZ) 𓆪\n\n"
    "`.كرر` + عـدد + كلمـه\n"
...
@sbb_b.zed_cmd(pattern="تعبير مكرر$")
...
@sbb_b.zed_cmd(pattern="ايقاف التكرار ?(.*)")
...
@sbb_b.zed_cmd(pattern=f"{UNSPAM} ?(.*)",)
async def spammer(event):
    reply = await event.get_reply_message()
    await event.delete()
    delgvar("spamwork")

...
ZelzalPRO_cmd = (
    "𓆩 [𝑠𝑜𝑢𝑟𝑐𝑒 𝑐𝑎𝑙𝑖𝑝ℎ  - اوامـر الاذا؏ـــة](t.me/ZZZ7iZ) 𓆪\n\n"
...
@sbb_b.zed_cmd(pattern="الاذاعه")
async def cmd(zelzallll):
    await edit_or_reply(zelzallll, ZelzalPRO_cmd)


@sbb_b.zed_cmd(pattern=f"للكروبات(?: |$)(.*)")
...
@sbb_b.zed_cmd(pattern=f"للمجموعات(?: |$)(.*)")
async def gcast(event):
    zedthon = event.pattern_match.group(1)
    if zedthon:
        msg = zedthon
...
async def pmto(event):
    ...
    if msg == "":
        return
    try:
        await sbb_b.send_message(chat_id, msg)
